    ...
    if (confirmBuy) {
      if (valuePilihan === "Pulsa") {
        ...
        if (Number(spliceMoney[0] >= 10)) {
          alert(`Total harga pulsa Rp. ${Number(spliceMoney[0]) + 2 + "." + spliceMoney[1]},- ke nomor ${noTelp.val()} sedang di proses. Terima kasih :)`);
        } else {
          alert(`Total harga pulsa Rp. ${Number(spliceMoney[0]) + 3 + "." + spliceMoney[1]},- ke nomor ${noTelp.val()} sedang di proses. Terima kasih :)`);
        }
      }
    } else {
      confirmBuy = confirm(`Pembelian paket data ${nominalValue} ke nomor ${noTelp.val()}`);
      if (confirmBuy) {
        alert(`Pembelian pulsa ${nominalValue} ke nomor ${noTelp.val()} sedang di proses. Terima kasih :) `);
      }
    }
  });
});
